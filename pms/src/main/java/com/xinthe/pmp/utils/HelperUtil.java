	...
	public static Supplier<List<PmsCollection>> pmsCollection = () ->
	{
		...
		List<Section> sectionList = Arrays.asList(section1, section2);
			return Arrays.asList(
					...
					PmsCollection.builder().id("1").screenId("Binay").screenLabelId("Label1")
							.viewTemplateId("Template1").createdDate(new Date()).updatedDate(new Date())
							.createdBy("User1").updatedBy("User1").sections(sectionList)
							.showOnWeb(true).showOnMobile(false).build(),

					PmsCollection.builder().id("2").screenId("John").screenLabelId("Label2")
							.viewTemplateId("Template2").createdDate(new Date()).updatedDate(new Date())
							.createdBy("User2").updatedBy("User2").sections(sectionList)
							.showOnWeb(true).showOnMobile(false).build());
	};
}
